#!/usr/bin/python
from metallograph_segmentation.models.multiview import model as px
import os
import datetime
import torch
import pandas as pd
...
config_file = "models/Multiview/config/MVUnetLinknet.config" if len(sys.argv) <= 1 else sys.argv[1]
conf = config.get_config(config_file)
print("==> CURRENT CONFIGURATION:\n", str(conf), file=sys.stderr)

modeldir = conf.get("modeldir", conf.get("save_d ir", "experiments"))
...
name = os.path.join(modeldir, conf.get("name",
    f"multiview_{conf['dataset']}_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"))
conf["kernel_size"] = 3
if "MV" in name:
...
if conf.get("run_cv", False):
    ...
    eval_val = crossval.crossval(x_total, y_total, dict(
        conf), px.train_Multiview, px.predict_Multiview, px.evaluate_Multiview, name=name, k=conf.get("kfold"),folds=folds)
...
#     (model, (loss, val_loss)), _, test_predictions = crossval.train_test(x_train, y_train, dict(conf), px.train_Multiview, 
#         px.predict_Multiview, x_test=x_test, validation_ratio=conf.get("validation_ratio"), name=name)
