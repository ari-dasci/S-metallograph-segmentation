...
class Multiview(nn.Module):
    ...
    def __init__(self, n_features, n_labels, kernel_size, dropout_rate, padding=0):
...
def get_modeloFeatures(modelo,backbone,nclasses):
    aux_params = {
        "weights_file": modelo,
        "backbone": backbone,
    ...
    # model.segmentation_head[1] => UpSampling (en su defecto, Identity)
    model.segmentation_head = model.segmentation_head[1]
    return model

def concatenateModelsFeatures(x, models, backbones):
    # inps will be a stack of features
    inps = None

    print('\n')
    print('Modelos a concatenar: ', models)
    print('\n')
    ...
    for modelName,backbone in zip(models, backbones):
        ...
        model = get_modeloFeatures(modelName, backbone, parametersModel.get("num_classes"))
...
def train_Multiview(params: dict, x: np.ndarray, y: np.ndarray, x_val=None, y_val=None, **kwargs):
    ...
    x = concatenateModelsFeatures(x, modelos, backbones)
    x = np.swapaxes(x, 2, 3)
    x = np.swapaxes(x, 1, 2)
    y = np.swapaxes(y, 2, 3)
    y = np.swapaxes(y, 1, 2)
    x_val = concatenateModelsFeatures(x_val, modelos, backbones)
    ...
    print('\n')
    print('\n')
    print('ConcatenatedFeaturesShapes en CPU: ', x.shape)
    print('\n')
    print('\n')

    model = Multiview(n_features=x.shape[1], n_labels=num_classes, kernel_size=kernel_size, dropout_rate=dropout_rate,
                      padding=padding)
    ...
    print('\n')
    print('\n')
    print('ConcatenatedFeaturesShapes en GPU: ', x.shape)
    print('\n')
    print('\n')
...
def predict_Multiview(result, x: np.ndarray, params: dict):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model, _ = result
    num_classes = int(params.get("num_classes", 4))
...
def evaluate_Multiview(model, x: np.ndarray, y: np.ndarray, params: dict):
    _ ,(tr_loss,val_loss) = model
    preds = predict_Multiview(model, x, params)
    eval_dict = evaluation(y, preds, int(params.get("num_classes", 4)))
    eval_dict.update(loss=tr_loss, val_loss=val_loss)
    return eval_dict

# def load_Multiview(weights_file, modelosBase, nclasses=4, **kwargs):
#     model = get_Multiview(len(modelosBase), nclasses)
