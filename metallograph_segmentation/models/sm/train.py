...
name = os.path.join(modeldir, conf.get("name",
    f"sm_{conf['model']}_{conf['dataset']}_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"))
...
if conf.get("run_cv", False):
    ...
    eval_val = crossval.crossval(x_total, y_total, dict(
        conf), px.train_sm, px.predict_sm, px.evaluate_sm, name=name, k=conf.get("kfold",5),folds=folds)
...
if conf.get("run_test", False):
    ...
    (model, (loss, val_loss)), _, test_predictions = crossval.train_test(x_train, y_train, dict(conf), px.train_sm, 
        px.predict_sm, x_test=x_test, validation_ratio=conf.get("validation_ratio"), name=name)
    ...
    predictions.save_images(
        test_predictions, f"{name}/{os.path.basename(name)}_test_#.png",colors =colors)
